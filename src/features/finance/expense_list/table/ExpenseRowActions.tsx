...
const ExpenseRowActions: FC<ExpenseRowActionsProps> = ({ expense, isEditing, handlers }) => {
	const { handleSaveEdit, handleCancelEdit, handleStartEdit, handleDelete } = handlers;
	...
	return (
		<div className="flex items-center justify-center gap-1">
			{isEditing ? (
				...
				<>
					<button
						onClick={() => !expense.isAutoGenerated && handleStartEdit(expense)}
					...
					<button
						onClick={() =>
							handleDelete(expense.id, expense.name, expense.isAutoGenerated || false)
						}
						disabled={expense.isAutoGenerated}
						title={expense.isAutoGenerated ? "Cannot delete" : "Delete"}
					>
						<Trash2 size={14} />
					</button>
				</>
