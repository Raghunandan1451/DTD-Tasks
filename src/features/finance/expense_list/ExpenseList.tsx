// src/features/finance/expense_section/ExpenseList.tsx
...
interface ExpenseListProps {
	expenses?: ExpenseEntry[]; // Optional prop for when passed from parent
}
...
const ExpenseList: FC<ExpenseListProps> = ({ expenses: propExpenses }) => {
	...
	const dailyTotal = useSelector(selectSelectedDateTotal);
	...
	const filteredExpensesSelector = useMemo(
		() => selectFilteredExpensesForSelectedDate(selectedGroup),
		[selectedGroup]
	);
	...
	const finalExpenses = useMemo(() => {
		if (propExpenses) {
			// Filter prop expenses by selected date and group
	...
	// Calculate final daily total
	const finalDailyTotal = useMemo(() => {
		if (propExpenses) {
			return propExpenses
		...
		return dailyTotal;
	}, [propExpenses, selectedDate, dailyTotal]);
	...
	return (
		<div className="space-y-4">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				...
				{/* Compact Pagination Controls - Between filters */}
				{totalPages > 1 && (
			...
			<div className="mt-4 border-t border-white/20 pt-2 text-right text-sm text-gray-700 dark:text-gray-300">
				Total for{" "}
