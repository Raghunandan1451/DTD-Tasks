...
export interface TableHandlers<T> {
	...
	/**
	 * `isProtected` (e.g. an auto-generated salary row that can't be
	 * deleted) is optional here since not every table has protected
	 * rows -- BudgetSimulatorTable always passes `false`, while
	 * ExpenseTable derives it from `expense.isAutoGenerated`.
	 */
	handleDelete: (id: string, name: string, isProtected?: boolean) => void;
	/**
	 * Widened to accept undefined quantity/unit -- ExpenseEntry has
	 * both as optional fields, so callers need to pass possibly-missing
	 * values without a separate cast or wrapper.
	 */
	displayQuantity?: (
